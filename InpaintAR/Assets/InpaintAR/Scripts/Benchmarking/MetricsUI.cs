...
namespace InpaintAR.Scripts.Benchmarking {
    public class MetricsUI : MonoBehaviour {
        ...
        public static InpaintingAlgorithms? CurrentAlgorithm;
        ...
        void Update() {
            // Toggle visibility with A button on Quest 3 controller
            if (OVRInput.GetDown(OVRInput.Button.One)) {
                m_isVisible = !m_isVisible;
                m_uiObj.SetActive(m_isVisible);
            }
        ...
        private void UpdateMetricUI() {
            string text = $"Current Algorithm: {CurrentAlgorithm}\n"; 
            text += $"Current FPS: {PerformanceEvaluator.GetTotalFPS():F2}\n";
            ...
            text += $"Average Clutter Reduction: {ClutterEvaluator.GetAverageClutterReduction():F2}%\n";
